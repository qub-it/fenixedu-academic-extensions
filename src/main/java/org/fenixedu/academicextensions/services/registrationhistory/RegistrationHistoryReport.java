...
public class RegistrationHistoryReport implements Comparable<RegistrationHistoryReport> {
	...
	public int compareTo(final RegistrationHistoryReport o) {
		...
		final Comparator<RegistrationHistoryReport> byYear = (x, y) -> ExecutionYear.COMPARATOR_BY_BEGIN_DATE
				.compare(x.getExecutionYear(), y.getExecutionYear());
		final Comparator<RegistrationHistoryReport> byDegreeType = (x, y) -> x.getDegreeType()
				.compareTo(y.getDegreeType());
		final Comparator<RegistrationHistoryReport> byDegree = (x, y) -> Degree.COMPARATOR_BY_NAME
				.compare(x.getRegistration().getDegree(), y.getRegistration().getDegree());
		final Comparator<RegistrationHistoryReport> byDegreeCurricularPlan = (x, y) -> x.getStudentCurricularPlanName()
				.compareTo(y.getStudentCurricularPlanName());
		final Comparator<RegistrationHistoryReport> byRegistrationNumber = (x, y) -> x.getRegistrationNumber()
				.compareTo(y.getRegistrationNumber());
	...
	public boolean getHasPreviousReingressionIncludingPrecedentRegistrations() {
		return Stream
				.concat(Stream.of(registration),
						RegistrationServices.getPrecedentDegreeRegistrations(registration).stream())
				.flatMap(r -> r.getReingressions().stream())
				.filter(ri -> ri.getExecutionYear().isBefore(getExecutionYear())).count() > 0;
	}

	public LocalDate getEnrolmentDate() {

		final Optional<RegistrationDataByExecutionYear> dataByYear = registration
				.getRegistrationDataByExecutionYearSet().stream()
	...
	public String getStudentStatutesNames() {
		return getStudentStatutes().stream().map(s -> s.getType().getName().getContent())
				.collect(Collectors.joining(", "));
	}
			...
			if (beginInterval != null) {
				...
				if (endInterval == beginInterval) {
					dates = BundleUtil.getString(Bundle.ENUMERATION,
							beginInterval.getAcademicPeriod().getAbbreviatedName()) + " "
	...
	public boolean getHasEnrolmentsWithoutShifts() {
		...
		for (final ExecutionCourse executionCourse : getRegistration()
				.getAttendingExecutionCoursesFor(getExecutionYear())) {
			if (!executionCourse.getAssociatedShifts().isEmpty()
					&& registration.getShiftsFor(executionCourse).isEmpty()) {
				return true;
			}
		}
	...
	public boolean getHasAnyInactiveRegistrationStateForYear() {
		return getRegistration().getRegistrationStates(getExecutionYear()).stream()
				.anyMatch(s -> !s.getType().getActive());
	}
	...
	public List<ProgramConclusion> getProgramConclusions() {
		return getConclusionReports().keySet().stream().sorted(Comparator.comparing(ProgramConclusion::getName)
	...
	public ConclusionReport getPartialConclusion() {
		return getConclusionReports().values().stream()
				.filter(cr -> cr != null && !cr.getProgramConclusion().isTerminal()).findFirst()
				.map(b -> new ConclusionReport(b)).orElse(ConclusionReport.empty());
	}
	...
	public Integer getCurricularYear() {
		if (this.curricularYear == null) {
			final CurricularYearResult result = RegistrationServices.getCurricularYear(getRegistration(),
					getExecutionYear());
	...
	public Collection<EnrolmentEvaluation> getImprovementEvaluations() {
		return RegistrationServices.getImprovementEvaluations(getRegistration(), getExecutionYear(),
				ev -> !ev.isAnnuled());
	}

	public boolean getHasImprovementEvaluations() {
		return RegistrationServices.hasImprovementEvaluations(getRegistration(), getExecutionYear(),
				ev -> !ev.isAnnuled());
	}

	public boolean getHasAnnulledEnrolments() {
		return getStudentCurricularPlan().getEnrolmentsSet().stream()
				.filter(e -> e.getExecutionYear() == getExecutionYear()).anyMatch(e -> e.isAnnulled());
	}
	...
	public BigDecimal getExecutionYearSimpleAverage() {
		if (this.executionYearSimpleAverage == null) {
			this.executionYearSimpleAverage = RegistrationHistoryReportService
					.calculateExecutionYearSimpleAverage(this);
		}
	...
	public BigDecimal getExecutionYearWeightedAverage() {
		if (this.executionYearWeightedAverage == null) {
			this.executionYearWeightedAverage = RegistrationHistoryReportService
					.calculateExecutionYearWeightedAverage(this);
		}
	...
	public LocalDate getExecutionYearConclusionDate() {
		if (this.executionYearConclusionDate == null) {
			// WARNING: is approved should be enough, but unfortunately there are some cases
			// (like in FM) where an enrolment only has evaluations with improvements
			final Enrolment enrolment = getEnrolments().stream()
					.filter(i -> i.isApproved() && i.calculateConclusionDate() != null)
	...
	public Integer getTotalNumberOfRegistrations() {
		return getRegistration().getStudent().getRegistrationsSet().size();
	}
	...
	public String getRegistrationObservations() {
		...
		return registration == null ? null
				: registration.getRegistrationObservationsSet().stream().map(o -> o.getUpdatedBy() + ":" + o.getValue())
						.collect(Collectors.joining(" \n --------------\n "));
	}
	...
	public LocalDate getDateOfBirth() {
		return Optional.ofNullable(getPerson()).map(p -> p.getDateOfBirthYearMonthDay()).map(dt -> dt.toLocalDate())
				.orElse(null);
	}
	...
	public String getEmergencyContact() {
		final Person person = getPerson();
		return Optional.ofNullable(person).map(p -> p.getProfile()).map(up -> up.getEmergencyContact())
				.map(ec -> ec.getContact()).orElse(null);
	}
	...
	public BigDecimal getEnrolmentYearsForPrescription() {
		...
		final Collection<ExecutionYear> executionYears = config.filterExecutionYears(registration,
				getEnrolmentExecutionYearsIncludingPrecedentRegistrations());
		...
		for (final ExecutionYear iter : executionYears) {
			bonification = bonification
					.add(config.getBonification(StatuteServices.findStatuteTypes(getRegistration(), iter),
	...
	public Boolean getCanPrescribe() {
		...
		final BigDecimal studentEcts = RegistrationServices
				.getCurriculum(registration, executionYear.getNextExecutionYear()).getSumEctsCredits();

		final int enrolmentYearsForPrescription = getEnrolmentYearsForPrescription().intValue();

		Comparator<? super PrescriptionEntry> comparator = (x, y) -> x.getEnrolmentYears()
				.compareTo(y.getEnrolmentYears());

		final PrescriptionEntry biggestEntryValue = config.getPrescriptionEntriesSet().stream()
		...
		final PrescriptionEntry entry = config.getPrescriptionEntriesSet().stream()
				.filter(e -> enrolmentYearsForPrescription <= e.getEnrolmentYears().intValue()).sorted(comparator)
				.findFirst().orElse(null);

		if (entry == null) {
			throw new AcademicExtensionsDomainException(
					"error.RegistrationHistoryReport.prescriptionConfig.is.missing");
		}
	...
	private Set<ExecutionYear> getEnrolmentExecutionYears() {
		return RegistrationServices.getEnrolmentYears(registration).stream()
	...
	public String getOtherConcludedRegistrationYears() {
		...
		getStudent().getRegistrationsSet().stream()
				...
				.forEach(r -> {
					...
					final SortedSet<ExecutionYear> executionYears = Sets
							.newTreeSet(ExecutionYear.COMPARATOR_BY_BEGIN_DATE.reversed());
		...
		return result.toString().endsWith("|") ? result.delete(result.length() - 1, result.length()).toString()
				: result.toString();
	}
	...
	private ExternalCurriculumGroup getAffinityCycleCurriculumGroup() {
		...
		return studentCurricularPlan.getExternalCurriculumGroups().isEmpty() ? null
				: studentCurricularPlan.getExternalCurriculumGroups().iterator().next();
	}
